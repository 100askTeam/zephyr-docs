...
观察下图的上下两部分：电脑构建时，软件配置确定启用哪些功能，设备树与 Binding 转成代码可用的硬件参数，再结合选中的应用、内核和驱动源码生成固件。烧录后，开发板 CPU 执行这些代码，应用通过 GPIO API 调用驱动，由驱动操作 GPIO 控制器。

![构建期的文件组合与运行期的驱动调用](./images/application-build-and-runtime.svg)

*图 1：LED 程序的构建输入与运行调用。上半部分表示配置、硬件描述和源码怎样参与构建；下半部分区分 CPU 执行的代码与实际硬件，P400 的输出电平控制 D12。*

这份分工决定了修改位置：闪烁间隔写在应用中，LED 接线写在设备树中，是否启用 GPIO 功能由配置决定。图中的“设备树生成头文件”保存构建过程生成的宏，代码通过这些宏取得连接信息；运行时使用的是编译进固件的参数和驱动，不会重新读取 DTS 或 Binding。后续编写 `main.c` 时会用到这条关系。
