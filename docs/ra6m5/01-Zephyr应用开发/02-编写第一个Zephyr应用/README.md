...
上一章已经找到应用、板卡描述和共享驱动的位置。现在在 `apps/board_bringup/` 中编写一个应用：启动时通过串口打印 Zephyr 版本与板卡目标，然后让板载用户 LED 每隔 500 ms 切换一次亮灭状态。这颗 LED 的器件编号是 D12，后文的 LED 操作都针对它。

先建立能够编译的串口应用，再使用 Board 已有的 `led0` 描述加入 GPIO 操作。所有命令从 `RA6M5/` 工程根目录执行；编译、烧录和串口操作参阅[编译与烧录程序](../../00-学习准备/03-编译与烧录程序/README.md)。
...
# 使用面向调试的优化级别，便于单步执行和查看变量。
CONFIG_DEBUG_OPTIMIZATIONS=y
```

`prj.conf` 是应用的 Kconfig 配置片段。Zephyr 把它与板卡默认配置等输入合并，最终配置保存在构建目录的 `zephyr/.config` 中。

`CONFIG_DEBUG_OPTIMIZATIONS=y` 让编译器使用 `-Og` 优化级别，减少变量被优化掉、源码单步跳行的情况，便于按[使用 VS Code 调试程序](../../00-学习准备/04-使用VSCode调试程序/README.md)中的步骤检查 GPIO 配置结果。这个选项定义在 `zephyr/Kconfig.zephyr` 中，构建后可以在生成的 `.config` 中确认它是否生效。
...
启动信息中的 `CONFIG_BOARD_TARGET` 来自本次构建选择的板卡目标，用于确认当前固件对应的板卡；更换构建目标后，输出会随配置改变。
...
前两项应为 `y`，说明 `prj.conf` 里的配置已进入最终结果；`CONFIG_BOARD_TARGET` 对应启动信息即将打印的板卡目标。此时 `main()` 只有打印和返回，尚未操作 LED。构建成功后再增加 GPIO 部分，后续出现的问题就能集中到新增代码。
...
| `0` | 控制器内的引脚编号 | P400 |
| `GPIO_ACTIVE_LOW` | 低电平表示有效状态 | 用户 LED（D12）在低电平时点亮 |
...
`GPIO_OUTPUT_INACTIVE` 表示配置成输出，并以逻辑无效状态启动。它不是固定的低电平：本板使用 `GPIO_ACTIVE_LOW`，因此初始输出是高电平，用户 LED 熄灭。设备树标志会由 `gpio_pin_configure_dt()` 与调用参数一起交给 GPIO 驱动。
...
![用户 LED（D12）的输出电平、亮灭状态和500毫秒翻转间隔](./images/led-toggle-timing.svg)
...
本次在开发板上运行 `board_bringup`，串口实测的应用启动信息为：

```text
Dshan RA6M5 bring-up: Zephyr 4.4.2 on dshan_ra6m5/r7fa6m5bf2cbg
```

启动信息中的版本与板卡目标来自该次构建。复位后继续核对输出与实物现象：

- 串口出现以 `Dshan RA6M5 bring-up: Zephyr` 开头的应用启动信息，后面包含本次构建的内核版本与板卡目标。
- 没有出现 `led0 controller is not ready` 或 `led0 configure failed` 错误。
- 板载用户 LED（D12）约每隔 500 ms 改变一次亮灭状态，一个完整亮灭周期约为 1 s。

应用只在启动时打印一次启动信息，循环中不会持续打印文字。因此启动信息之后串口保持安静、LED 继续闪烁，是这份程序的预期行为。若在烧录复位之后才打开串口，可能错过启动信息；保持监视窗口打开，再复位一次即可观察。

| 当前现象 | 优先检查的位置 |
| --- | --- |
| 构建找不到 `src/main.c` | `CMakeLists.txt` 的源文件路径与实际目录 |
| 构建报错涉及 `DT_ALIAS(led0)` 或 `gpios` | 所选 Board，以及生成设备树中的别名和节点 |
| 启动信息出现，但提示控制器未就绪 | 最终 `.config` 中的 GPIO 配置，以及驱动初始化结果 |
| 没有启动信息，但用户 LED 正在闪烁 | 串口是否已经打开、是否错过启动输出、是否需要复位 |
| 串口有启动信息，但用户 LED 不闪烁 | 是否构建并烧录了加入循环后的 `board_bringup`，以及 GPIO 配置错误输出 |
