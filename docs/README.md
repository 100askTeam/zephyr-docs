...
:::tip[已经下载完整工程包]
一期完整工程包已经包含 `.west/`、Zephyr 源码、`sdk_env/` 工具链，以及 `tools/` 中的 Python、west 和 Git。解压后保持目录结构，**不需要创建或激活 `.venv`**；先完成板卡、JTAG 和串口检查，再按一期课程构建应用。二期从空目录搭建工程时，才按[二期环境准备](./common-01-准备开发环境/README.md?phase=phase2)安装工具并创建虚拟环境。
:::
