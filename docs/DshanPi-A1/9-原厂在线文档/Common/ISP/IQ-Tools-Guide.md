...
| 版本号 | 作者 | 修改日期 | 修改说明 |
| --- | --- | --- | --- |
| V2.0.0 | 陈煜 | 2021-06-22 | 适配v2.0.0版本 |
| V2.0.1 | 徐苏皖 | 2021-07-09 | 适配v2.0.1版本，新增IQ文件在线导入功能 |
| v2.0.2 | 陈煜 | 2021-07-10 | 3.1.1小节增加重置相机应用脚本相关操作说明 |
...
| 芯片名称 | 系统平台 | ISP版本 |
| --- | --- | --- |
| RK356x | Linux/Android | RKISP21 |
| RK3588 | Linux/Android | RKISP30 |
| RV1106 | Linux/Android | RKISP32 |
...
Raw Capture：离线raw抓取，包含相机参数配置和曝光控制两部分：

相机参数配置：提供了拍图所需的sensor曝光参数、模块/光源名称标记、分辨率和增益/曝光参数范围等；

Online Capture：在线数据流抓取，支持raw 和yuv 两种格式；
...
2. 运行rkaiq\_tool\_server和其他环境配置工作（其中执行命令和文件传输可通过：TFTP、串口、ADB等协议或工具完成，示例操作中使用的是ADB）

#### 3.1.1 Android系统平台

##### 3.1.1.1 RK356X & RK3588
...
1. 在SDK中编译获取rkaiq\_tool\_server和rkaiq\_3A\_server  

2. SDK中的路径: external/rkaiq\_tool\_server, 固件必须是user-debug版本  
...
5. 将rkaiq\_tool\_server和rkaiq\_3A\_server推入板端/vendor/bin/路径下  

6. 进入/vendor/bin/路径，执行修改权限命令：chmod 755 rkaiq\_tool\_server(以及  
...
注意：使用New功能生成的初版IQ参数推入设备后应使用如下操作重置相机应用，重新运行ToolServer
...
-s: 指定YUV采集用的video节点，默认值会自动搜索iqtool采集节点，也可以手动配置为mainPath或  

selfPath  

-r: RTSP推流功能开关，0为关闭，1为开启，默认值为0，开启后可以在CaptureTool中使用Start RTSP  
...
3. 点击OK按钮，将会加载对应平台的调试界面，如3-2-2所示；
...
3. 注意：这里离线帧使用的是.rkraw格式，内部数据为compact+align256，其他格式可以使用RKRAW Tool进行批量转换，采集Raw时也请将config.ini的RKRawV2配置为1（在线采集Raw的方法请参考3.7.2小节的描述）
